...
describe("data.api", () => {
  ...
  it("DataController 核心接口均使用 RAW response boundary", async () => {
    ...
    await expect(getAllDeviceDataSummaries()).resolves.toBe(responses[1]);
    await expect(getDevicePointSummaries("device-1")).resolves.toBe(responses[2]);
    await expect(resetAdaptiveConfig("device-1")).resolves.toBe(responses[3]);
    await expect(getPointHistory("device-1", "point-1", { limit: 10 })).resolves.toBe(responses[4]);
    await expect(getRecentAlarms({ level: "WARN" })).resolves.toBe(responses[5]);
    await expect(getDeviceAlarmHistory("device-1", { pointCode: "temperature" })).resolves.toBe(responses[6]);
