...
describe("alarm-utils", () => {
  ...
  it("格式化主告警页展示文本", () => {
    expect(alarmCurrentValue({ current_value: 42 })).toBe("42");
    expect(alarmCurrentValue({ alarmValue: "" })).toBe("-");
    expect(alarmLevelText("CRITICAL")).toBe("严重");
    expect(alarmLevelText("WARNING")).toBe("警告");
  });
});
