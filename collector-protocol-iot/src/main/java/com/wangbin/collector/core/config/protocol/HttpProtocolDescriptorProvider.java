    ...
    public void register(ProtocolDescriptorRegistry registry) {
        registry.registerPrimary(registry.descriptor("HTTP", "HTTP",
                ...
                registry.fields(
                        ...
                        registry.field("insecureSkipVerify", "boolean", "显式跳过 HTTPS 服务端证书校验（不安全）", false, "false",
                                List.of("true", "false"), "security"),
                        registry.field("path", "string", "DIRECT 相对请求路径（url 可含基础路径；完整 URL 留空）；旧 apiPrefix 仅兼容读取", false, "", null, "request"),
                        ...
                        registry.field("receiveMethod", "select", "Receive method", false, "GET",
                                List.of("GET", "POST", "PUT", "DELETE", "HEAD"), "request"),
                        registry.field("writeEndpoint", "string", "DIRECT 写入端点（写入时必填）", false, "", null, "request"),
                        registry.field("writeMethod", "select", "DIRECT 写入方法", false, "POST",
                                List.of("GET", "POST", "PUT", "DELETE", "HEAD"), "request"),
                        registry.field("writeAckMode", "select", "RESPONSE=明确成功 ACK；HTTP_2XX=空响应仅表示已接收", false, "RESPONSE",
                                List.of("RESPONSE", "HTTP_2XX"), "response"),
                        registry.field("healthCheckPath", "string", "可选连接探测路径（空值不探测）", false, "", null, "advanced"),
                        registry.field("heartbeatEndpoint", "string", "可选心跳路径（空值不发请求）", false, "", null, "advanced"),
                        ...
                        registry.field("authMethod", "select", "Auth method", false, "POST",
                                List.of("GET", "POST", "PUT", "DELETE", "HEAD"), "security"),
                        registry.field("proxyHost", "string", "Proxy host", false, "", null, "advanced"),
                        registry.field("proxyPort", "number", "Proxy port (required with proxyHost)", false, "", null, "advanced"),
