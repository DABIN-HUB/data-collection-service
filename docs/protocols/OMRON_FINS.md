# OMRON FINS 协议实现与设计记录

> 当前状态（2026-07-17）：FINS/UDP 内存区读写、连续块合并、批量回退已经实现；`executeCommand`
> 已按白名单支持控制器信息、CPU 状态和时钟读取，并校验 SID、命令回显和 End Code。FINS 无原生点位订阅，
> 订阅请求会明确拒绝或按显式策略降级轮询。本文后续历史设计内容继续保留，未完成项以此状态说明和
> `docs/99-问题与未完成功能清单.md` 为准。
